...
const FieldMatcher: React.FC<FieldMatcherProps> = ({ files, onMappingChange, onMerge }) => {
  ...
  useEffect(() => {
    ...
    files[0].contacts.forEach((contact: Contact) => {
    ...
    files[1].contacts.forEach((contact: Contact) => {
      Object.keys(contact).forEach((key) => {
        if (!mandatoryFields.includes(key)) {
          allFields2.add(key);
        }
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-base text-gray-500">
        Match fields from the first file to fields in the second file. BEGIN, END, and VERSION are automatically matched. Fields with the same name are preselected.
      </p>
      <div className="grid grid-cols-2 gap-4">
        <div className="col-span-1">
          <h3 className="text-lg font-medium text-gray-900 mb-2">{files[0].name}</h3>
        </div>
        <div className="col-span-1">
          <h3 className="text-lg font-medium text-gray-900 mb-2">{files[1].name}</h3>
        </div>
        {fields1.map((field1) => (
          <div key={field1} className="col-span-2 flex items-center space-x-2">
            <label className="w-1/2 text-sm font-medium text-gray-700">{field1}</label>
            <select
              className="w-1/2 mt-1 block pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md"
